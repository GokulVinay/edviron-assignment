...
function Dashboard() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      console.error("Error during fetching of transactions:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCopySchoolId = async (schoolId) => {
    try {
      ...
      setTimeout(() => alert("school id was successfully copied"), 500);
    } catch (err) {
      console.error("Failed to copy the respective text: ", err);
    }
  };
...
<h1 style={{color:'red'}}>Transactions Overview</h1>
...
<p className="loading-text"> Transaction is Loading...</p>
</div>
)}
